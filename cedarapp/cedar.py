...
import re, math, hashlib, time, random, string
import numpy as np
import pandas as pd
...
for k in res1:
    ...
    kiq["options"] = list(np.array(ap)[np.random.choice(len(ap), len(ap), replace=False)])
    opt1.append(kiq)
...
def cedar_h1questions():
    output = list(np.array(opt1)[np.random.choice(len(opt1), maxH1, replace=False)])
    return json.dumps(output)

@cedar.route("/cedar/h2questions")
def cedar_h2questions():
    output = list(np.array(opt2)[np.random.choice(len(opt2), maxH2, replace=False)])
    return json.dumps(output)
...
def cedar_h1submit():
    ...
    if len(rows) < maxH1*10: return json.dumps({"success": 0})
    output = pd.DataFrame.from_dict(rows, orient="index")
    output = output[["qid", "optid", "value", "user"]]
    output = output.reset_index()
    rn = ''.join(random.choice(string.ascii_uppercase + string.digits) for _ in range(strN))
...
def cedar_h2submit():
    ...
    for k in request.form:
        ...
        val = request.form[k]
        rows[k] = {"qid": qid, "optid": optid, "user": _user, "value": val}
    if len(rows) < maxH2*5: return json.dumps({"success": 0})
    output = pd.DataFrame.from_dict(rows, orient="index")
    output = output[["qid", "optid", "value", "user"]]
    output = output.reset_index()
    rn = ''.join(random.choice(string.ascii_uppercase + string.digits) for _ in range(strN))
