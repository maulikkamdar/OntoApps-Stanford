...
def train_graph(sess, saver, g, train_df, test_df, batch_size = 64, num_epochs = 10, iterator = BucketedDataIterator):
    ...
    while current_epoch < num_epochs:
        ...
        #break
        bds = pd.concat([batch[1], batch[2], batch[3], pd.Series(scores[:,1]).to_frame(name="scores")], axis=1)
        if train_set_df.shape[0] == 0: train_set_df = bds
        else: train_set_df = pd.concat([train_set_df, bds], axis=0)
        accuracy += accuracy_
        losses.append(loss)
        if tr.epochs > current_epoch:
            train_set_df.to_csv("train_epoch" + str(current_epoch+1) + "_set_preds.tsv", sep="\t", index=None)
            train_set_df = pd.DataFrame()
            validation_set_df = pd.DataFrame()
            current_epoch += 1
            ...
            with open('iterators.pickle', 'w') as f:  # Python 3: open(..., 'wb')
                pickle.dump([tr, te], f)
            validation_set_df.to_csv("validation_epoch" + str(current_epoch) +"_set_preds.tsv", sep="\t", index=None)
